...
function scene:createScene( event )
	...
	level = display.newText("Level 25",40,10,'marker felt', 15)
	group:insert(level)
  ...
  wood.anchorY = 1
	...
	cupA = display.newImageRect('cupV.png', 70, 93)
  ...
  cupA.anchorY = 1
	...
	lineT1 = display.newRect( 38,h/2-22, 39, 3)
	lineT1.name = "lineT1"
	...
	lineL1 = display.newRect(-17+30, h/2-50, 60, 1  )
	lineL1.name = "lineL1"
	lineL1.rotation= -80
	...
	lineR1 = display.newRect( 64, h/2-50, 60, 1  )
	lineR1.name = "lineR1"
	lineR1.rotation= 80
  ...
  cupC.anchorY = 1
	...
	lineT3 = display.newRect( 104,h-55, 39, 3)
	lineT3.name = "lineT3"
	...
	lineL3 = display.newRect(78,h-20, 65, 1  )
	lineL3.rotation= 80
	...
	lineR3 = display.newRect(128,h-20, 65, 1  )
	lineR3.rotation= -80
  ...
  woodD.anchorY = 1
	...
	cupB = display.newImageRect('cupV.png', 70, 93)
	cupB.anchorX = -1
  	cupB.anchorY = 1
	cupB:setFillColor(0, 0, 255)
	cupB.x =  w-70
	cupB.y = h/2+20
	group:insert(cupB)
	...
	lineT2 = display.newRect(w-44,h/2-22, 39, 3)
	lineT2.name = "lineT2"
	...
	lineL2 = display.newRect(w-65, h/2-50, 55, 2  )
	lineL2.name = "line1"
	lineL2.rotation= -80
	...
	lineR2 = display.newRect(w-16, h/2-50, 58, 2  )
	lineR2.name = "linet"
	lineR2.rotation= 80
	...
	bluBack = display.newRect(w/2,h/2, 50, 7) 
	bluBack.name = "bluBack"
	--bluBack:setReferencePoint(display.CenterReferencePoint)
	bluBack:setFillColor(0, 0, 255) 
	physics.addBody( bluBack, "static", { isSensor = true } )
	group:insert(bluBack)
	
	greenBack = display.newRect(w/2 ,h/2-140, 50, 7) 
	greenBack.name = "bluBack"
	--greenBack:setReferencePoint(display.CenterReferencePoint)
	greenBack:setFillColor(255,0,0) 
	physics.addBody( greenBack, "static", { isSensor = true } )
	group:insert(greenBack)
  ...
  cupD.anchorY = 1
	...
	lineTD = display.newRect( w-114,h-55, 39, 3)
	lineTD.name = "lineTD"
	lineTD.anchorX = -1
  lineTD.anchorY = 1
	...
	lineLD = display.newRect(w-120, h-20, 70, 1 )
	lineLD.rotation= 80
	...
	lineRD = display.newRect(w-70, h-20, 70, 1 )
	lineRD.rotation= -80
...
function start()
   ...
   function menuButtons()   	
    ...
    function hintButton1()
      hhints2 = display.newImageRect('hint25.png',w ,h)
      hhints2.x = w - w/2
      hhints2.y = h - h/2
...
function transitionA()
	if j < 200 then
				storyboard.star = 3
        		print ("3star")
        		myStars25:store('stars25',13)
        		myStars25:save()
    	elseif j < 250 then
    			storyboard.star = 2
        		print ("2star")
        		myStars25:store('stars25',12)
        		myStars25:save()
    	elseif j < 300 then
    			storyboard.star = 1
        		print ("1star")
        		myStars25:store('stars25',11)
        		myStars25:save()
    end
    z=1
	...
	function greenBack:collision (event)
		if (event.other.name == 'ball') then
           event.other:setFillColor(255,0,0)
           event.other.name = 'ballG'
     elseif (event.other.name == 'ball1') then
           event.other:setFillColor(255,0,0)
           event.other.name = 'ballG'
        end
	end		
		greenBack:addEventListener("collision", greenBack)		
