...
function scene:createScene( event )
	...
	level = display.newText("Level 16",30,10,'marker felt', 20)
	group:insert(level)
	
	circle = display.newCircle(30, 50, 25)
	circle.name = 'circle'
  ...
  group:insert(floor1)
	...
	stone = display.newImageRect('vertical-wood.png', 20, h-50)
  ...
  cupC.anchorY = 1
	...
	lineT3 = display.newRect(  w-44,h/2+18, 49, 3)
	lineT3.name = "lineT3"
	...
	lineL3 = display.newRect(w-95+30, h/2+48, 65, 1  )
	lineL3.name = "lineL3"
	lineL3.rotation= 80
	...
	lineR3 = display.newRect( w-50+34, h/2+48, 65, 1  )
	lineR3.rotation= -80
...
function start()
   ...
   function menuButtons()   	
    ...
    function hintButton1()
      hhints2 = display.newImageRect('hint16.png',w ,h)
      hhints2.x = w - w/2
      hhints2.y = h - h/2
...
function transitionA()
	if j < 100 then
				storyboard.star = 3
        		print ("3star")
        		myStars16:store('stars16',13)
        		myStars16:save()
    	elseif j < 120 then
    			storyboard.star = 2
        		print ("2star")
        		myStars16:store('stars16',12)
        		myStars16:save()
    	elseif j < 140 then
    			storyboard.star = 1
        		print ("1star")
        		myStars16:store('stars16',11)
        		myStars16:save()
    end
     z=1
    ...
    ball[j].y = 40
      --  ball[j] = display.newCircle( ranX, 40, 2 )
       ball[j]:setFillColor(255,0,255)
        ...
        physics.addBody(ball[j],"dynamic", {bounce=.9, radius = 7.5,density = .3,  friction=1 })
        group:insert(ball[j])
        j = j + 1
